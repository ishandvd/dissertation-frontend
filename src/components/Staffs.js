...
const Staffs = ({numLines, x, y, width, lineGap, staffGap}) => {
    ...
    let annotations = [
      <text key="hihat_text" textAnchor="middle" 
              x={x - 20} 
              y={y + 5}>Hihat</text>,
      <text key="snare_text" textAnchor="middle" 
              x={x - 20} 
              y={y + 5 + staffGap}>Snare</text>,
      <text key="kick_text" textAnchor="middle" 
              x={x - 20} 
    ...
    for (let i = 0; i < numLines; i++) {
        ...
        lines.push(
        <text key={"seconds_text_" + i} textAnchor="middle" 
              x={x + width + 10} 
              y={y + i * lineGap - 5}>{5 * (i+1)}s</text>
        )
        // Draw the staff lines in each line block
        lines.push(
        <line key={"top_ledger_" + i} x1={x} y1={y + i * lineGap} 
              x2={x + width} y2={y + i * lineGap} 
              stroke="black" strokeWidth="2" />)
        lines.push(
          <line key={"mid_ledger_" + i} x1={x} y1={y + (i * lineGap) + staffGap} 
                x2={x + width} y2={y + (i * lineGap) + staffGap} 
                stroke="black" strokeWidth="2" />)
        lines.push(
          <line key={"bot_ledger_" + i} x1={x} y1={y + (i * lineGap) + (staffGap * 2)} 
